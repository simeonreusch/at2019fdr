...
for INTERVAL in INTERVALS:
    ...
    # print(fitresult)
    # By hand epoch 0 (no extinction)
    # fitresult["temp1"] = 12000
    # fitresult["scale1"] = 1.2e+23
    # extinction_av = 0.000001
    # extinction_rv = 4

    # By hand epoch 1
    # fitresult["temp1"] = 12296.6159
    # fitresult["scale1"] = 3.7e+23
    # extinction_av = 0.000001
    # extinction_rv = 4
    ...
    luminosity_1, _, _, _ = utilities.calculate_bolometric_luminosity(
        temperature=fitresult["temp1"],
        scale=fitresult["scale1"],
        redshift=REDSHIFT,
    ...
    luminosity_2, _, _, _ = utilities.calculate_bolometric_luminosity(
        temperature=fitresult["temp2"],
        scale=fitresult["scale2"],
        redshift=REDSHIFT,
        temperature_err=None,
    ...
    if not plotmag:
        ...
        ax1.plot(
            nu,
            unextincted_spectrum_1.flux * nu,
            color="tab:blue",
            linestyle="dotted",
            linewidth=0.6,
            label=rf"spectrum 1 unextincted",
        )
        ...
        if FIT == 3:
            ...
            ax1.plot(
                nu,
                unextincted_spectrum_2.flux * nu,
                color="tab:red",
                linestyle="dotted",
                linewidth=0.6,
                label=rf"spectrum 2 unextincted",
            )
            ax1.plot(
                nu,
                combined_spectrum.flux * nu,
                color="black",
                # linestyle="dotted",
    ...
    annotation = f"lumin. opt/UV: {luminosity_1:.1e}\nlumin. IR: {luminosity_2:.1e}\nlumin. total: {total_luminosity:.1e}"
    ax1.text(
        0.35,
        0.9,
        annotation,
    ...
    if plotmag:
        outfile = os.path.join(PLOTDIR, f"double_bb_mag_{INTERVAL}.png")
    else:
        outfile = os.path.join(PLOTDIR, f"double_bb_nufnu_{INTERVAL}.png")
