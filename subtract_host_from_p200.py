...
infile_from_epoch_1 = os.path.join(
    LIGHTCURVE_DIR, "galfit_result_twocomp_fromepoch1.csv"
)
infile_from_epoch_4 = os.path.join(
    LIGHTCURVE_DIR, "galfit_result_twocomp_fromepoch4.csv"
)
...
for band in ["H", "J", "Ks"]:
    for obj in ["host", "psf"]:
        df_1[f"{band}_abmag_{obj}"] = utilities.p200_vega_to_ab(
            vegamag=df_1[f"{band}_vegamag_{obj}"].values, band=f"P200+{band}"
        )
        df_4[f"{band}_abmag_{obj}"] = utilities.p200_vega_to_ab(
            vegamag=df_4[f"{band}_vegamag_{obj}"].values, band=f"P200+{band}"
        )
        df_1[f"{band}_flux_{obj}"] = utilities.abmag_to_flux(
            df_1[f"{band}_abmag_{obj}"].values, magzp=0
        )
        df_4[f"{band}_flux_{obj}"] = utilities.abmag_to_flux(
            df_4[f"{band}_abmag_{obj}"].values, magzp=0
        )
    ...
    df_1[f"{band}_abmag_comb"] = utilities.flux_to_abmag(
        df_1[f"{band}_flux_comb"].values, flux_nu_zp=0
    )
    df_4[f"{band}_abmag_comb"] = utilities.flux_to_abmag(
        df_4[f"{band}_flux_comb"].values, flux_nu_zp=0
    )
    ...
    infile_host_spectrum = os.path.join(
        HOST_MODEL_DIR, "Tywin_parasfh_wduste_spectrum_NEW.dat"
    )
    ...
    df[f"{band}_flux_transient_err"] = (
        df[f"{band}_flux_err"] + df[f"{band}_flux_synthetic_err"]
    )
    ...
    df[f"{band}_abmag_err_transient"] = utilities.flux_err_to_abmag_err(
        df[f"{band}_flux_transient"], df[f"{band}_flux_transient_err"]
    )
...
for band in ["J", "H", "Ks"]:
    for i, obsmjd in enumerate(df.obsmjd.values):
        ...
        abmag_err = df.query(f"obsmjd == {obsmjd}")[
            f"{band}_abmag_err_transient"
        ].values[0]
        abmags.append(abmag)
...
output_df = pd.DataFrame(
    columns=["band", "telescope", "obsmjd", "mag", "mag_err", "alert"]
)
