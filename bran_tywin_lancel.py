...
def plot_lightcurve(
    ...
    df = df_bran
    fig = plt.figure(figsize=(0.8 * FIG_WIDTH, 0.5658 * FIG_WIDTH), dpi=DPI)
    filter_wl = utilities.load_info_json("filter_wl")
    plt.subplots_adjust(bottom=0.13, top=0.85, left=0.13, right=0.9)
    ...
    i = 0
    redshifts = [REDSHIFT_BRAN, REDSHIFT_TYWIN, REDSHIFT_LANCEL]
    labels = ["AT2019dsg", "AT2019fdr", "AT2019aalc"]

    for df in [df_bran, df_tywin, df_lancel]:
    ...
    ax1.axvline(
        t_neutrino_bran.mjd, linestyle=":", label="IC191001A", color="blue", linewidth=2
    )
    ax1.axvline(
