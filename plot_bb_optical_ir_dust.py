...
bb_spectrum, bb_opt_flux = utilities.blackbody_spectrum(
    temperature=bb_temp,
    scale=bb_scale,
    redshift=REDSHIFT,
...
ir_bb_specrum, bb_ir_flux = utilities.blackbody_spectrum(
    temperature=1500,
    scale=bb_scale / 13000,
    redshift=REDSHIFT,
    get_bolometric_flux=True,
)
...
bolo_lumi_combined = bolo_lumi_bb_opt + bolo_lumi_bb_ir
increase = 100 / bolo_lumi_bb_opt * bolo_lumi_combined - 100

print(f"bolometric luminosity optical component: {bolo_lumi_bb_opt:.2e}")
print(f"bolometric luminosity IR dust component: {bolo_lumi_bb_ir:.2e}")
print(f"bolometric luminosity combined: {bolo_lumi_combined:.2e}")
print(f"combined luminosity is {increase:.2f}% higher than optical alone")

# bb_opt_lumi = utilities.calculate_luminosity(bb_spectrum, 100, 1e9, REDSHIFT)
# bb_ir_lumi = utilities.calculate_luminosity(ir_bb_specrum, 100, 1e9, REDSHIFT)

# print(bb_opt_lumi)
# print(bb_ir_lumi)

# bolo_lumi_bb_otp = utilities.calculate_bolometric_luminosity(
...
texts = ["IR dust BB", "optical BB with extinction", "combined spectrum"]
position = [(1.1e14, 3.8e-13), (2.4e14, 1.8e-13), (2.7e14, 1e-12)]
...
if not plotmag:
    ...
    ax2.set_xlabel(r"Energy [eV]", fontsize=FONTSIZE)
...
if plotmag:
    outfile = os.path.join(PLOTDIR, "dust_ir_optical_mag.png")
else:
    outfile = os.path.join(PLOTDIR, "dust_ir_optical_flux.png")
